var _ = require("underscore")._, async = require("async"), fs = require("fs"),
    exec = require("child_process").exec, config = require("../config.json");
  ...
  var _get_wifi_info =
    ...
    async.series([
      function run_ifconfig(next_step) {
        run_command_and_set_fields("ifconfig wlan0", ifconfig_fields,
                                   next_step);
      },
      function run_iwconfig(next_step) {
        run_command_and_set_fields("iwconfig wlan0", iwconfig_fields,
                                   next_step);
      },
    ],
                 function(error) {
      ...
      _reboot_wireless_network =
          function(wlan_iface, callback) {
        async.series([
          function down(next_step) {
            exec("sudo ifconfig " + wlan_iface + " down",
                 function(error, stdout, stderr) {
                   if (!error)
                     console.log("ifconfig " + wlan_iface +
                                 " down successful...");
                   next_step();
                 });
          },
          function up(next_step) {
            exec("sudo ifconfig " + wlan_iface + " up", function(error, stdout,
                                                                 stderr) {
              if (!error)
        ...
        _is_ap_enabled(function(error, result_addr) {
          ...
          async.series([
            ...
            function update_interfaces(next_step) {
              write_template_to_file("./assets/etc/dhcpcd/dhcpcd.ap.template",
                                     "/etc/dhcpcd.conf", context, next_step);
            },

            // Enable the interface in the dhcp server
            function update_dhcp_interface(next_step) {
              write_template_to_file("./assets/etc/dnsmasq/dnsmasq.ap.template",
                                     "/etc/dnsmasq.conf", context, next_step);
            },
            ...
            function update_hostapd_conf(next_step) {
              write_template_to_file(
                  ...
                  "/etc/hostapd/hostapd.conf", context, next_step);
            },
              ...
              exec("sudo systemctl restart dnsmasq",
                   function(error, stdout, stderr) {
                     if (!error)
                       ...
                       console.log("... dnsmasq server failed! - " + stdout);
                     next_step();
                   });
            },
          ...
          async.series([
            ...
            function update_wpa_supplicant(next_step) {
              write_template_to_file(
                  ...
                  "/etc/wpa_supplicant/wpa_supplicant-wlan0.conf", connection_info,
                  next_step);
            },
            ...
            function update_interfaces(next_step) {
              write_template_to_file(
                  ...
                  "/etc/dhcpcd.conf", connection_info, next_step);
            },
            ...
            function update_dhcp_interface(next_step) {
              write_template_to_file(
                  ...
                  "/etc/dnsmasq.conf", connection_info, next_step);
            },
            ...
            function update_hostapd_conf(next_step) {
              write_template_to_file(
                  ...
                  "/etc/hostapd/hostapd.conf", connection_info, next_step);
            },
